...
module.exports = {
  ...
  data: new ContextMenuCommandBuilder()
    .setName('Purge After This Message')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (filteredMessages.size === 0) {
        return interaction.reply({ content: 'No messages found to purge after the selected message.', ephemeral: true });
      }
      ...
      const embed = new EmbedBuilder()
        .setColor("#0ffc03")
        .setDescription(`<:success:${successId}> **Successfully deleted ${filteredMessages.size} messages after the selected message**`)
        .setTimestamp();

      await interaction.reply({ embeds: [embed], ephemeral: true });
    } catch (error) {
      console.error('Error purging messages after the selected message:', error);
